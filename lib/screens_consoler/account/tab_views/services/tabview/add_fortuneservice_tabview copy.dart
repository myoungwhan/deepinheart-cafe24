...
class _AddFortuneserviceTabviewState extends State<AddFortuneserviceTabview> {
                ...
                (package) => {
                  ...
                  'new_price':
                      (package.coins *
                          ...
                          .round(),
                  'description':
                      '${package.duration} ${'-minute consultation x'.tr} ${'consultation'.tr} x ${package.session} ${'sessions'.tr}',
                },
              )
              .toList();
    } else {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Display selected hashtags as blue chips with cross buttons
            ...
            Wrap(
              ...
              children:
                  allHashtags.map((hashtag) {
                    // Check if this hashtag is from fetched API list or user-added
                    final isFetched = fetchedHashtags.any(
                      (fetched) => fetched.id == hashtag.id,
                    );
                        ...
                        children: [
                          ...
                          GestureDetector(
                            onTap: () {
                              setState(() {
                                if (isFetched) {
                                  // For fetched hashtags, remove from fetchedHashtags
                                  fetchedHashtags.removeWhere(
                                    (fetched) => fetched.id == hashtag.id,
                                  );
                                } else {
                                  // For user-added hashtags, remove from selectedHashtags
                  ...
                  onPressed: () {
                    if (_packageNameController.text.isNotEmpty) {
                      ...
                      var packageData = {
                        ...
                        'new_price': newPrice,
                        'description':
                            '${_packageDurationController.text}-minute consultation x ${_packageSessionController.text} sessions',
                      };
  ...
  List<int> _getSelectedHashtagIds() {
    // Get all hashtags (fetched from API + user-added)
    final allHashtags = [...fetchedHashtags, ...selectedHashtags];

    // Return IDs of all hashtags that are currently displayed (selected)
    return allHashtags.map((hashtag) => hashtag.id).toList();
  }
